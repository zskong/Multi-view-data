...
%     dlmwrite(txtpath, strcat('Dataset:',cellstr(dataName), '  Date:',datestr(now)),'-append','delimiter','','newline','pc');
    ...
    for ichor = 1:length(anchor)
        for id = 1:length(d)
            ...
            tic;
            [A,W,Z,G,F,iter,obj,alpha] = algo_OMMSC(X,Y,lambda(j),d(id),anchor(ichor)); % X,Y,lambda,d,numanchor
            [~,idx]=max(F);
            ...
            %save([matpath,'/',matname],'Z','A','W','alpha','G','F');
            save([matname],'Z','A','W','alpha','G','F');
